...
```{r}
jobs_data <- read_csv("../laerer_test_renset.csv")
```
...
```{r}
Lok_excel <- read_excel('/work/208886/Ikke-danske_lokationer.xlsx')
```
...
```{r}
folkeskole_register <- read_excel("../Registerdata fra alle grundskoler/Folkeskoler.xlsx")
```
...
```{r}
danske_kommuner <- read_excel("kommune_postnumre.xlsx", col_types = "text")
```
...
![kommune_kolonne.](kommune_kolonne.png)
...
![kommune_hjemmeside.](kommune_hjemmeside.png)
...
```{r}
kommune_missing <- read_excel("kommune_missing.xlsx")
```
...
```{r}
kode_df <- read_excel('/work/208886/Samletkodeliste-udenaar.xlsx')
```
...
![Ordbogsmetoden.](ordbogsmetoden.png)
